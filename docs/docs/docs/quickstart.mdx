...
Build declarative CLIs that support streaming and
piping out of the box using simple function closures.
...
function myCommand() {
  // cli recieves process.argv.slice(2)
...
```javascript title="~/my-project/index.mjs"
export function myCommand(flags) {
  return function cli([greet, name]) {
    console.log(flags)
    return `${greet}, ${name}!`
  }
